...
class TF_Luna:
    def __init__(self, port="/dev/serial0"):
        ...
        if platform.system() == "Windows":
            ...
            self.lidar_port = MockSerial(
                port, self.BAUD_RATE, self.SERIAL_TMEOUT)
        else:
            # Use real serial class on Raspberry Pi
            if serial is None:
                raise RuntimeError(
                    "serial module is not available on this platform")
            self.lidar_port = serial.Serial(
                port, self.BAUD_RATE, timeout=self.SERIAL_TMEOUT)
    ...
    def get_distance_to_obstacle(self):
        """Reads distance from LiDAR with timeout"""
        ...
        while time.time() - start_time < timeout:
            ...
            if count > 8:
                ...
                if bytes_data[0] == 0x59 and bytes_data[1] == 0x59:
                    ...
                    if strength < 100:
                        return 8.0  # Return max distance for low signal
        ...
        print("LiDAR timeout - no data received")
        return 0
